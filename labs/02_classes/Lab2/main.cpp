...
int main() {
    ...
    std::cout << "testing constructors" << std::endl;
    Time x(0,0,1); //checking 0 hours case
    Time y(12,24,39); // checking 12 hours
    Time z(23,59,59);//last second check

    std::cout << "testing accessors" << std::endl;
    std::cout << "x: " << x.getHour() << " " << x.getMinute() << " " << x.getSecond() << std::endl;
    std::cout << "y: " << y.getHour() << " " << y.getMinute() << " " << y.getSecond() << std::endl;
    std::cout << "z: " << z.getHour() << " " << z.getMinute() << " " << z.getSecond() << std::endl;
    std::cout << std::endl;
    assert (x.getHour() == 0);
    assert (y.getMinute() == 24);
    assert (z.getSecond() == 59);
    x.PrintAMPM();
    y.PrintAMPM();
    z.PrintAMPM();
}
