...
private:
  ...
  TreeNode<T>* copy_tree(TreeNode<T>* old_root) {
    ...
    if(old_root== NULL){
      ...
      TreeNode<T>* node = new TreeNode<T>(old_root->value);
         node->left = copy_tree(old_root->left);
         node->right = copy_tree(old_root->left);
    return node;
  }
...
//  }
  ...
  iterator find(const T& key_value, TreeNode<T>* p) {
        while((p->left || p->right) && p->value!=key_value) {
            if(p->value < key_value){
                p = p->right;
            } else if(p->value > key_value){
                p = p->left;
