...
public:   
  ...
  //print function
  void print (Vec<T>& v) {
      std::cout<<"Number of elements stored in the vector: "<<m_size<<std::endl;
      std::cout<<"Number of array locations allocated,  m_size <= m_alloc: "<<m_alloc<<std::endl;
...
template <class T> int remove_matching_element (Vec<T>& v, const T& t) {
    int num = 0;
